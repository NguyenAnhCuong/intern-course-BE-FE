...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (users.length > 0) return res.status(400).json({ message: 'Email đã tồn tại' });
    ...
    res.status(500).json({ message: 'Lỗi máy' });
  }
});
...
router.post('/login', async (req, res) => {
  ...
  try {
    const [users] = await db.promise().query('SELECT * FROM users WHERE email = ?', [email]);
    if (users.length === 0) return res.status(400).json({ message: 'Sai email hoặc mật khẩu' });
...
router.post('/grant', verifyToken, requireAdmin, async (req, res) => {
  ...
  if (!user_id || !device_id) return res.status(400).json({ message: 'Thiếu user_id hoặc device_id' });
