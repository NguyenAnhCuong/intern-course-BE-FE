...
router.post('/devices', async (req, res) => {
  ...
  try {
    await pool.query('INSERT INTO devices (id, name, status) VALUES (?, ?, ?)', 
...
router.put('/devices/:id', async (req, res) => {
  ...
  try {
    const [result] = await pool.query('UPDATE devices SET name = ?, status = ? WHERE id = ?', 
...
router.get('/iot/data', async (req, res) => {
  ...
  let query = 'SELECT * FROM iot_data';
  let params = [];
...
router.post('/alerts', async (req, res) => {
  ...
  try {
    await pool.query('INSERT INTO alerts (device_id, message, type) VALUES (?, ?, ?)', 
