...
async function sendEmailAlert(data) {
  const mailOptions = {
    ...
    text: `Thiết bị ${data.deviceId} báo nhiệt độ/gas cao: ${data.temperature || data.gas} lúc ${data.timestamp}`,
  };
...
mqttClient.on('message', async (topic, message) => {
  try {
    ...
    await pool.query(
      ...
      [data.deviceId, data.temperature || null, data.gas || null, formattedTimestamp]
    );

    console.log(`💾 Dữ liệu lưu vào bảng ${tableName}`);

    // Gửi email nếu vượt ngưỡng
    if ((data.temperature && data.temperature > 35) || (data.gas && data.gas > 80)) {
